!>@brief MPI infrastructure module for ARES.
...
contains
  ...
  !> Gather one real(R8) per rank onto root. `arr` must be at least mpi_size_
  !> long on rank 0; only rank 0's copy is meaningful on return.
